...
type UserMessage = Anthropic.MessageParam;
type PromptFn = (data: Record<string, unknown>) => UserMessage;

export const SYSTEM_PROMPT = `Sen Menusflow'un AI Menü Danışmanısın. Türkiye'deki restoran, kafe ve pastane sahiplerine veriye dayalı, eyleme dönüştürülebilir öneriler verirsin.
...
export const PROMPTS: Record<string, PromptFn> = {
  language_mismatch: (d) =>
    userMsg(
      `Kural: language_mismatch (menüde olmayan baskın bir dil var).
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: eksik dile çeviri eklemek.`,
  ...
  bounce_point: (d) =>
    userMsg(
      `Kural: bounce_point (müşteriler tek bir kategoride menüyü bırakıp gidiyor).
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: bu kategorideki ürünleri (görsel/fiyat/açıklama) gözden geçirmek.`,
  ...
  dead_product: (d) =>
    userMsg(
      `Kural: dead_product (son ${d.weeks} haftadır neredeyse hiç görüntülenmeyen ürün).
Ürün: "${d.itemName}"
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: ürünü menüden kaldırmak veya görseli/fiyatı/sırasını değiştirmek.`,
  ...
  category_promotion: (d) =>
    userMsg(
      `Kural: category_promotion (bir kategori menüde önde ama sıralamada geride).
Kategori: "${d.categoryName}"
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: bu kategoriyi menünün üst sırasına taşımak.`,
  ...
  hourly_pattern: (d) =>
    userMsg(
      `Kural: hourly_pattern (günün belirli bir saatinde menü trafiği patlıyor).
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: bu saatte bu kategoriye yönelik kampanya/kombo açmak.`,
  ...
  seasonal_alert: (d) =>
    userMsg(
      `Kural: seasonal_alert (menü uzun süredir güncellenmedi, mevsim değişti).
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: mevsime uygun yeni ürünler/kampanya eklemek.`,
  ...
  scan_decline: (d) =>
    userMsg(
      `Kural: scan_decline (haftalık QR menü taramaları belirgin şekilde düşüyor).
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: QR kodun masada/girişte görünür olup olmadığını kontrol etmek, sosyal medyada hatırlatmak.`,
  ...
  long_dwell_item: (d) =>
    userMsg(
      `Kural: long_dwell_item (müşteri tek bir ürün sayfasında aşırı uzun kalıyor; karar veremiyor veya bilgi yetersiz).
Ürün: "${d.itemName}"
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: ürün açıklamasını netleştirmek, fiyat/porsiyon/içerik bilgisini eklemek veya daha iyi bir görsel koymak.`,
  ...
  new_product_underperforming: (d) =>
    userMsg(
      `Kural: new_product_underperforming (yakın zamanda eklenen ürün diğer ürünlerden az ilgi görüyor).
Ürün: "${d.itemName}"
...
Türkçe tek başlık + iki cümlelik öneri üret. Aksiyon: ürünü daha üst sıraya almak, kapak görseli kullanmak veya kısa süreli tanıtım fiyatı uygulamak.`,
  ...
  price_increase_opportunity: (d) =>
    userMsg(
      `Kural: price_increase_opportunity (talebi belirgin biçimde artan, fiyatı revize edilebilecek ürün).
Ürün: "${d.itemName}"
